...
Public Class Form1
    ...
    Sub SaveData()
        Dim text As String = """Location"",""Pointers"",""Byte Count"",""String Value"",""New String Value""" + Environment.NewLine

        For i As Integer = 0 To ListView1.VirtualListSize - 1
            Dim item As ListViewItem = ListView1.Items(i)
            text += """" + item.Text + """,""" + item.SubItems(1).Text + """,""" + item.SubItems(2).Text + """,""" + item.SubItems(3).Text + """,""" + item.SubItems(4).Text.Replace("""", """""").Replace(vbLf, "") + """" + Environment.NewLine
        Next

        My.Computer.FileSystem.WriteAllText(sggg_data, text, False)
    End Sub


    Sub CalculateUnusedSpace()
        Dim LOLA As String = Nothing
        Dim LOLAn As Int64 = 0
        Dim LOLAFirst As String = Nothing

        Dim SeccondA As Int64 = 0
        Dim FirstA As Int64 = 0
        Dim FinalString As String = Nothing
        Dim SizeCount As Int64 = 0
        ...
        For i As Integer = 0 To ListView1.VirtualListSize - 1
            ...
            Dim NewText As String = item.SubItems(4).Text
            ...
            If NewText.Length > 0 Then
                If NewText.Length > Bytes Then
                    If item.SubItems(1).Text <> Nothing Then 'check pointer presence
                        ...
                        If SeccondA = 0 Then
                            FirstA = Convert.ToInt64(DefaultLocation, 16)
                            SeccondA = Convert.ToInt64(DefaultLocation, 16) + (Bytes + 2)
                            SizeCount = (Bytes + 2)

                            FinalString += FirstA.ToString("X5") + " "

                        Else
                            If SeccondA >= Convert.ToInt64(DefaultLocation, 16) Then
                                SeccondA = Convert.ToInt64(DefaultLocation, 16) + (Bytes + 2)
                                SizeCount += (Bytes + 2)

                            ElseIf SeccondA + 2 >= Convert.ToInt64(DefaultLocation, 16) Then
                                SeccondA = Convert.ToInt64(DefaultLocation, 16) + (Bytes + 2)
                                SizeCount += (Bytes + 4)
                            Else

                                FinalString += SeccondA.ToString("X5") + " (" + SizeCount.ToString + ")" + Environment.NewLine
        ...
        My.Computer.FileSystem.WriteAllText(IO.Path.Combine(IO.Directory.GetCurrentDirectory, "LOLA3.txt"), FinalString, False)
